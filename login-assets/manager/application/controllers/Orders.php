...
// 		$this->load->library('session');
		...
		$this->fromEmail = 'adityakohli467@gmail.com';
// 		ini_set('display_errors', 1);x
...
// error_reporting(E_ALL);
	...
	{ 
           ...
           if($mailType=='todayDelivery'){
            ...
            $this->email->from($this->fromEmail, 'Bizadmin ');
            $this->email->subject('Bizadmin ');
...
// error_reporting(E_ALL);    
			...
			$data['unapprovedQuotes'] = $dashboardDataForOrders['unapprovedQuotes'];
// 			$data['tommorowDeliveries'] = $dashboardDataForOrders['tommorowAllDeliveries'];
		...
		if($data['unapprovedQuotes']){
		    $data['unapprovedQuotesClassName'] = 'border-red';
		}else{
		   $data['unapprovedQuotesClassName'] = ''; 
		}
...
// 			echo "<pre>"; print_r($frontend_orders_tommorow); exit;
...
// 	Quote
	...
	public function new_quote_save()
	{
		if(!empty($this->session->userdata('username')))
		{
			...
			if(isset($_POST['shipping_method']) && $_POST['shipping_method'] == 2){
...
// 		else echo "Oops! You do not have the correct authentication token to see this invoice. Please get in touch with us at the earliest to resolve this issue.";
	...
	public function payment_process($branch_id='')
	{
	 if($branch_id !=''){
	   $this->session->set_userdata('branch_id',(int)$branch_id); 
	   $this->session->set_userdata('user_id',(int)$branch_id);
...
// 		echo $this->db->last_query();exit;
		...
		$status= "Modified";
			...
			$status = "Paid";
				...
				if((isset($row->order_status_id) &&  $row->order_status_id == 2) || (isset($row->order_status) &&  $row->order_status == 2)){
...
// 		$elem="<form id=\"securepay_form\" action=\"https://test.payment.securepay.com.au/secureframe/invoice\" method=\"post\">";
		...
		$elem.="<input type=\"hidden\" name=\"return_url\" value=\"".base_url()."index.php/orders/payment_process/".$branch_id."\">";
		$elem.="<input type=\"hidden\" name=\"return_url_target\" value=\"parent\">";
		$elem.="<input type=\"hidden\" name=\"cancel_url\" value=\"".base_url()."index.php/orders/order_history\">";
		$elem.="<input type=\"hidden\" name=\"callback_url\" value=\"".base_url()."index.php/orders/payment_process/".$branch_id."\">";
     ...
     if($order_info['order_status'] == 3){
      ...
      $PrdctExcludedFromGST_DeductItsPrice = 0;
		foreach($order_prods as $order_prod){
		    ...
		    $allproducts[] = array(
		   'name' => strip_tags($order_prod->product_name),
		   'order_product_comment' => strip_tags($order_prod->order_product_comment),
		   'quantity' => $order_prod->quantity,
		   'price' => $order_prod->price,
		   'total' => $order_prod->total,
		   'option' => $order_prod->options,
		   'product_description'=> strip_tags($mini_desc),
	...
	$coupon_discount=($order_info['coupon_discount']*($total_so_far))/100;
     ...
     $coupon_discount = 0;
        ...
        $pdf->SetFont('Montserrat', '',6);
         ...
         $pdf->SetFont("Montserrat", '', 9);
         $pdf->Cell(10, 4, '  1300 117 909', 0, 0);
     ...
     // Header
       ...
       $pdf->SetTextColor(247, 190, 104);
        ...
        if($order_info['shipping_gate_number'] != ''){
            $pdf->MultiCell(190, 5,'Gate No: '.$order_info['shipping_gate_number'],0,'R'); $lineGap = $lineGap +1.5;
            
        }
        if($order_info['shipping_building_number'] != ''){
            $pdf->MultiCell(190, 5,'Building number: '.$order_info['shipping_building_number'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_department_name'] != ''){
            $pdf->MultiCell(190, 5,'Department name: '.$order_info['shipping_department_name'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_level_of_building'] != ''){
            $pdf->MultiCell(190, 5,'Level of building: '.$order_info['shipping_level_of_building'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_room_number'] != ''){
            $pdf->MultiCell(190, 5,'Room number: '.$order_info['shipping_room_number'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_business_name'] != ''){ 
            $pdf->MultiCell(190, 5,'Business Name: '.$order_info['shipping_business_name'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_street_number'] != ''){
            $pdf->MultiCell(190, 5,'Street Number: '.$order_info['shipping_street_number'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_delivery_contact_name'] != ''){
            $pdf->MultiCell(190, 5,'Delivery contact name: '.$order_info['shipping_delivery_contact_name'],0,'R'); $lineGap = $lineGap +1.5;
        }
        if($order_info['shipping_delivery_contact_number'] != ''){
            $pdf->MultiCell(190, 5,'Delivery contact number: '.$order_info['shipping_delivery_contact_number'],0,'R'); $lineGap = $lineGap +1.5;
        }
       
        $pdf->SetLeftMargin(10);
        $finalTopGap = $lineGap + $initialValue;
        //--- end--------------------------------------------------------------------------

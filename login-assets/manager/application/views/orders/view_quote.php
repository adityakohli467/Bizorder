...
<div class="container_full bg_img_banner mt-5">
			...
			<div class="container-fluid">
				<div class="row">
					...
					<div class="col-12">
							...
							<div class="card-body">
								<div class="row">
									<div class="col-12">
										...
										<button class="btn btn-warning" onclick="open_modal()">Approve Email <i class="fa fa-send"></i></button>
								     </div>
									</div>
								</div>
							</div>
							...
							<strong class="mr-2">Approval Mail Comments: </strong><?php echo $order_info->approval_comments;?><br>
                             <?php }  ?>
										...
										<td colspan="2" width="50%">
											...
											?><br><hr>
											<?php if(isset($order_info->pickup_delivery_notes) && $order_info->pickup_delivery_notes !='') {  ?>
								...
								if(!empty($order_products)){
									...
									<?php if(isset($order_info->delivery_fee) && $order_info->delivery_fee !=0){
									...
									<?php if(!is_null($order_info->coupon_id)){
										...
										<tr>
											...
											<td>$<?php echo number_format(($order_info->order_total+$order_info->late_fee+$delivery_fee-$coupon_discount)/11,2, '.', '');?></td>
										</tr>
										<tr>
											<td width="70%">&nbsp;</td>
						...
						<!--<tr><td><strong>Company Name: </strong></td><td><?php echo $company_name; ?></td></tr><tr><td><strong>ABN</strong></td><td><?php echo $company_abn; ?></td></tr></table>-->
						<!--					</td>-->
						<!--				</tr>-->
						<!--			</table>-->
								</div>
	...
	<script>
	function convertInvoice(order_id)
		{
			window.open("<?php echo base_url();?>index.php/orders/convertToInvoice/"+order_id);
		}
		
		function print_window(order_id)
		{
			window.open("<?php echo base_url();?>index.php/orders/print_quote/"+order_id);
		}
		function open_modal()
		...
		function send_mail()
		{
			...
			$.ajax({
				url:'<?php echo base_url();?>index.php/orders/send_quote_email/<?php echo $order_info->order_id;?>/<?php echo $fingerprint;?>/<?php $f = $order_info->oc_order_id?$order_info->oc_order_id.'-1800INV':$order_info->order_id.'-1800HC'; echo $f;?>',
				method:"POST",
				data:{

					"email":$("#email").val(),
                     "customer_name":'<?php echo $order_info->firstname;?>'

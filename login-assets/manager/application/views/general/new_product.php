				...
				<div class="row">
					...
					<div class="col-lg-5 col-sm-12 mb-3">
						<div class="card">
							...
							<div class="card-body">
								<form action="<?php echo base_url();?>index.php/general/add_new_product" method="POST" id="new_product_form" enctype='multipart/form-data'>
									<div class="form-group row">
										<label class="col-sm-3 col-form-label">Product Title</label>
										<div class="col-sm-9">
											<input type="text" class="form-control" name="product_heading" id="heading">
										</div>
									</div>
									<div class="form-group row">
					...
					<div class="col-lg-4 col-sm-12 mb-3">
						<div class="card">
							<div class="card-header">
								<p class="card-title" style="display:inline">Options</p><button class="btn btn-danger pull-right btn-sm" data-toggle="modal" data-target="#new-option-modal">New Option</button>
							</div>
							<div class="card-body">
							    <div class="table-responsive">
								<table class="table table-striped" id="all-options">
									...
									<tbody id="all-options-tbody">
										<?php foreach($all_options as $option){
											...
											echo "<td><button class=\"btn btn-info btn-sm\" onclick=\"add_option('".$option->option_value_id."','".$option->name."','".$option->option_value_id."')\"><i class=\"fa fa-plus\"></i></button></td>";
											echo "</tr>";
										}?>
									</tbody>
								</table>
...
{
	$("#option_id_"+product_option_id).remove();
	$("#option-hidden-"+op_id).remove();
}
function add_option(option_value_id,op_name,op_id)
{
	...
	else
	{
		...
		$("#current_product_options").append("<tr id=\"option_id_"+op_id+"\"><td>"+op_name+"</td><td>$"+$("#option-price-"+option_value_id).val()+"</td><td><button class=\"btn btn-sm btn-danger\" type=\"button\" onclick=\"remove_option('"+op_id+"')\"><i class=\"fa fa-remove\"></i></button></td></tr>");
		$("#new_product_form").append("<input type=\"hidden\" name=\"option_value["+op_id+"]\" id=\"option-hidden-"+op_id+"\" value=\""+$("#option-price-"+option_value_id).val()+"\">");
	}
}

...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		// Set tenantDb on the model from controller if available
		$CI = &get_instance();
		if (isset($CI->tenantDb) && is_object($CI->tenantDb)) {
			$this->ion_auth_model->tenantDb = $CI->tenantDb;
		}
	 ...
	 * @author Mathew
	 */
	public function ConditionalUserFtehcing($isActive=1){
	    $users = $this->ion_auth_model->ConditionalUserFtehcing($isActive);
	    return $users;
	...
	public function is_staff($id = FALSE)
	{
		...
		$admin_group = $this->config->item('staff_group', 'ion_auth');

...
class Auth extends MY_Controller
{
	...
	public function __construct()
	{
		...
		// Ensure tenantDb is set on ion_auth model after library loads
		if (isset($this->tenantDb) && is_object($this->tenantDb)) {
			$this->ion_auth->ion_auth_model->tenantDb = $this->tenantDb;
		}
...
// 		$this->form_validation->set_error_delimiters($nthis->config->item('error_start_delimiter', 'ion_auth'), $this->config->item('error_end_delimiter', 'ion_auth'));
		...
		$this->lang->load('auth');
	   // Only check user groups if user is logged in and tenantDb is available
	   if ($this->ion_auth->logged_in() && isset($this->tenantDb) && is_object($this->tenantDb)) {
   ...
   public function getTenantDbDetails($tenantIdentifier){
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			$password = trim($this->input->post('password')); // sanitize the pass before checking In dbbb

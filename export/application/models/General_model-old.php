...
class General_model extends CI_Model{
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_companies");
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_departments");
		foreach($query->result() as $row)
		{
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_product p JOIN tbl_product_description pd ON p.product_id=pd.product_id");
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_category_description cd JOIN tbl_category c ON cd.category_id=c.category_id");
		foreach($query->result() as $row)
		{
			$name=$this->db->escape($row->name);
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_product_to_category");
		foreach($query->result() as $row)
		{
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_option_description");
		foreach($query->result() as $row)
		{
			$name=$this->db->escape($row->name);
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_option_value_description ovd JOIN tbl_option_value ov ON ovd.option_value_id=ov.option_value_id");
		foreach($query->result() as $row)
		{
			$name=$this->db->escape($row->name);
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_product_option_value pov JOIN tbl_product_option po ON po.product_option_id=pov.product_option_id");
		foreach($query->result() as $row)
		{
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_order ord JOIN tbl_customer cust ON (ord.firstname=cust.firstname AND ord.lastname=cust.lastname)");
//		echo "<pre>".print_r($query->result(),1);exit;
	...
	{
		$query=$this->db->query("SELECT * FROM tbl_order_product");
		foreach($query->result() as $row)
		{
	...
	public function fetch_dash_data()
	{
		...
		$query=$this->db->query("SELECT count(*) as deliveries_today FROM orders WHERE delivery_date_time BETWEEN '".date("Y-m-d",strtotime('now'))." 00:00:01"."' AND '".date("Y-m-d",strtotime('now'))." 23:59:59"."' and  user_id =$userId");
		$res['deliveries_today']=$query->result()[0]->deliveries_today;
		$query=$this->db->query("SELECT count(*) as deliveries_month FROM orders WHERE delivery_date_time BETWEEN '".date("Y-m-d",strtotime('first day of this month'))." 00:00:01' AND '".date("Y-m-d",strtotime('last day of this month'))." 23:59:59'  and  user_id =$userId ");
		$res['deliveries_this_month']=$query->result()[0]->deliveries_month;
		$query=$this->db->query("SELECT sum(order_total) as revenue_month FROM orders WHERE delivery_date_time BETWEEN '".date("Y-m-d",strtotime('first day of this month'))." 00:00:01' AND '".date("Y-m-d",strtotime('last day of this month'))." 23:59:59'  and  user_id =$userId");
		$res['revenue_this_month']=number_format($query->result()[0]->revenue_month,2);
		$query=$this->db->query("SELECT sum(order_total) as total_unpaid FROM orders WHERE order_status!=3 and  user_id =$userId ");
		$res['total_unpaid']=number_format($query->result()[0]->total_unpaid,2);
...
where IF (o.user_id=0,up.user_id=$userId,o.user_id=$userId) ORDER BY o.order_id desc limit 5");
	...
	public function new_company($name,$phone,$abn,$address)
	{
		...
		$this->db->query("INSERT INTO company (company_name,company_phone,company_abn,company_address) VALUES (".$name.",".$phone.",".$abn.",".$address.")");
	}
	public function new_department($company_id,$department)
	{
		$department=$this->db->escape($department);
		$this->db->query("INSERT INTO department (company_id,department_name) VALUES (".$company_id.",".$department.")");
	}
	...
	public function add_new_product($product_heading,$product_name,$amount,$category,$description,$product_desc_1,$product_desc_2,$product_desc_3,$product_desc_4,$product_desc_5,$minimum,$options)
	{
		...
		$this->db->query("INSERT INTO product (product_name,product_description,product_desc_1,product_desc_2,product_desc_3,product_desc_4,product_desc_5,product_price,product_minimum,product_status) VALUES (".$this->db->escape($product_name).",".$description.",".$product_desc_1.",".$product_desc_2.",".$product_desc_3.",".$product_desc_4.",".$product_desc_5.",".$amount.",".$minimum.",0)");
	...
	public function update_customer($customer_id,$phone,$email,$cc)
	{
		$this->db->query("UPDATE customer SET customer_telephone=".$this->db->escape($phone).",customer_email=".$this->db->escape($email).",customer_cost_centre=".$this->db->escape($cc)." WHERE customer_id=".$customer_id);
	}
	public function update_company($company_id,$company_phone,$company_address,$abn)
	{
		$this->db->query("UPDATE company SET company_phone=".$this->db->escape($company_phone).",company_address=".$this->db->escape($company_address).",company_abn=".$this->db->escape($abn)." WHERE company_id=".$company_id);
	}

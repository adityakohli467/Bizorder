...
// Init list
var employeeList = new List("SupplierList", options).on("updated", function (list) {
    // console.log(list);
...
xhttp.onload = function () {
  ...
  json_records.forEach(raw => {
   ...
   employeeList.add({
      supplier_id: '<a href="javascript:void(0);" class="fw-medium link-primary">'+raw.supplier_id+"</a>",
      supplier_name: raw.supplier_name,
      supplier_status: isStatus(raw.status),
    });
    employeeList.sort('supplier_id', { order: "desc" });
    refreshCallbacks();
  });
  employeeList.remove("supplier_id", '<a href="javascript:void(0);" class="fw-medium link-primary">01</a>');
}
xhttp.open("GET", "configurationSupplierController");
xhttp.send();
 

    // isCount = new DOMParser().parseFromString(
    //   employeeList.items.slice(-1)[0]._values.supplier_id,
    //   "text/html"
    // );
...
var idField = document.getElementById("id-field"),
  ...
  departmentField = document.getElementById("department-field"),
  effectiveStartDate = document.getElementById("effective-start-date"),
...
document.getElementById("showModal").addEventListener("show.bs.modal", function (e) {
  if (e.relatedTarget.classList.contains("edit-item-btn")) {
    ...
    document.getElementById("user-com-wrap").style.display = "none";
    ...
    if(errFlag == 0){ 
        ...
        var Fdata = new FormData(form);
            ...
            if(this.responseText == 'success'){
                var msgIcon = 'success';
                var msgTitle = 'User added successfully!';
              }
              else{
                  var msgIcon = 'error';
   ...
   $('.invalid-field').css('display','none');
    ...
    if(errFlag == 0){ 
        ...
        var Fdata = new FormData(form);
            ...
            const xhttp = new XMLHttpRequest();
            xhttp.open("POST", "/userUpdateController");
            xhttp.onload = function() {
            ...
            if(this.responseText == 'success'){
                var msgIcon = 'success';
                var msgTitle = 'User updated successfully!';
              }
              else{
                  var msgIcon = 'error';
                  var msgTitle = 'Something went wrong!';
  ...
  removeBtns.forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      ...
      itemValues.forEach(function (x) {
        deleteid = new DOMParser().parseFromString(x._values.supplier_id, "text/html");
              ...
              const xhttp = new XMLHttpRequest();
                xhttp.onload = function() {
                    console.log(this.responseText);
                      ...
                      if(this.responseText == 'success'){
                          var msgIcon = 'success';
                          var msgTitle = 'User deleted successfully!';
                      }
                      else{
                          var msgIcon = 'error';
                          var msgTitle = 'Something went wrong!';
                  ...
                  var supplier_id = isdeleteid;
                  
                 
                  xhttp.open("GET", "userDeleteController?user_id="+supplier_id+"&user_status=0&user_role=3");
                  xhttp.send(); 
  ...
  editBtns.forEach(function (btn) {
    ...
    btn.addEventListener("click", function (e) {
      ...
      itemValues.forEach(function (x) {
        ...
        isid = new DOMParser().parseFromString(x._values.supplier_id, "text/html");
      console.log('x valyes'+x._values.email);
        ...
        if (selectedid == itemId) {
          ...
          effectiveStartDate.value = x._values.effectiveStartDate
        //   console.log('outer'+x._values.is_employee);
          if(x._values.is_employee == 'Yes'){
            //   console.log('inner'+x._values.is_employee);
              $('#formCheck6').attr('checked','checked');
          }
         let redirectPage = () => {
        ...
        window.location.href = url;
                ...
                //  const xhttp = new XMLHttpRequest();
                //   xhttp.open("GET", "employee-edit?supplier_id="+selectedid);
                //   xhttp.send(); 
...
function deleteMultiple() {
  ...
  items.forEach(function (ele) {
    if (ele.checked == true) {
      ...
      var supplier_id = trNode.querySelector('.supplier_id a').innerHTML;
      ids_array.push(supplier_id);
    }
  });
       ...
       document.getElementById("delete-record").addEventListener("click", function () {
              ...
              const xhttp = new XMLHttpRequest();
                ...
                //   var supplier_id = isdeleteid;
                  ...
                  xhttp.open("POST", "userMultipleDeleteController?user_id="+ids_array+"&user_status=0");
                  xhttp.send(); 

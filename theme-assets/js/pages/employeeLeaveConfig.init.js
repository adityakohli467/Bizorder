    ...
    if(this.responseText){
      ...
      json_records.forEach(raw => {
          ...
          var leaveStatusChange ='';
            ...
            leaveStatusChange += '<select class="form-control" name="leaveStatus" onchange="leaveStatusChange(this);" id="'+raw.leaveId+'" required />';
            leaveStatusChange += '<option value="1" '+enable+'>Enable</option><option value="0" '+disable+'>Disable</option></select>';
            
            if(raw.emp_role && raw.emp_role){
               if(raw.emp_role == 0){var emp_role_name = "All"; }else{ var emp_role_name = raw.emp_role_name; }
                tasksList.add({
                    leave_id: '<a href="apps-tasks-details" class="fw-medium link-primary">' + raw.leaveId + "</a>",
                    leave_type_name: raw.leave_type_name,
                    company_name: raw.company_name,
                    branch_name: raw.branch_name,
                    leave_allowed: raw.leave_allowed,
                    leave_period: raw.leave_period,
                    emp_role_name: emp_role_name,
                ...
                tasksList.add({
                    ...
                    leave_allowed: raw.leave_allowed,
                    leave_period: raw.leave_period,
                    leave_left: raw.leave_left,
                    
                });
            }
            
            tasksList.sort('leave_type_name', { order: "ASC" });
            
            refreshCallbacks();
        });
        tasksList.remove("leave_type_name",``);
    }
}
xhttp.open("GET", "/employeeLeaveConfigController");
xhttp.send();
...
var idField = document.getElementById("leaveId"),
    leave_type_name = document.getElementById("leave_type_name"),
    doc_required = document.getElementById("doc_required"),
    emp_role = document.getElementById("emp_role"),
    leave_allowed = document.getElementById("leave_allowed"),
    leave_period = document.getElementById("leave_period"),
...
document.getElementById("showModal").addEventListener("show.bs.modal", function (e) {
    if (e.relatedTarget.classList.contains("add-btn")) {
        document.getElementById("exampleModalLabel").innerHTML = "Add New Leave Type";
        document.getElementById("showModal").querySelector(".modal-footer").style.display = "block";
        document.getElementById("add-btn").style.display = "block";
    } else {
        document.getElementById("exampleModalLabel").innerHTML = "List Leave Type";
    ...
    e.preventDefault();
        ...
        //form submit
           $('.invalid-field').css('display','none');
            ...
            var leaveTypeName = leave_type_name.value;
            var docRequired = doc_required.value;
            var empRole = emp_role.value;
            var leaveAllowed = leave_allowed.value;
            var leavePeriod = leave_period.value;
            
            var errFlag = 0;
            
            if(leaveTypeName =='' ){
                errFlag = 1;
                $('#err-leave_type_name-field').css('display','block');
            }
            if(docRequired =='' ){
                errFlag = 1;
                $('#err-doc_required-field').css('display','block');
            }
            
            if(empRole =='' ){
                errFlag = 1;
                $('#err-emp_role-field').css('display','block');
            }
            if(leaveAllowed =='' ){
                errFlag = 1;
                $('#err-leave_allowed-field').css('display','block');
            } 
            if(leavePeriod =='' ){
                errFlag = 1;
                $('#err-leave_period-field').css('display','block');
            }
            ...
            if(errFlag == 0){
               var form = $('#form-event')[0];
                ...
                const xhttp = new XMLHttpRequest();
                xhttp.open("POST", "/employeeLeaveConfigAddController");
                xhttp.onload = function() {
                    console.log(this.responseText);
                      if(this.responseText == 'success'){
                          var msgIcon = 'success';
                          var msgTitle = 'Leave Added successfully!';
                      }
                      else{

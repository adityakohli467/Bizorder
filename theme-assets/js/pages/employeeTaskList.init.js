...
function deleteMultiple() {
  ...
  items.forEach(function (ele) {
    if (ele.checked == true) {
      ...
      var task_id = trNode.querySelector('.task_id a').innerHTML;
      ids_array.push(task_id);
    }
  });
  console.log('array task_id'+ids_array);
  ...
  if (typeof ids_array !== 'undefined' && ids_array.length > 0) {
    ...
    $('#deleteOrder').modal('show');
       document.getElementById("delete-record").addEventListener("click", function () {
              ...
              const xhttp = new XMLHttpRequest();
                xhttp.onload = function() {
                    console.log(this.responseText);
                      ...
                      if(this.responseText != 0 && this.responseText == 1){
                          var msgIcon = 'success';
                          var msgTitle = this.responseText+' Task deleted successfully!';
                      } else{
                          var msgIcon = 'error';
                          var msgTitle = 'Something went wrong!';
                      }
                ...
                //   var emp_id = isdeleteid;
                  ...
                  xhttp.open("POST", "TaskMultipleDeleteController?task_id="+ids_array+"&emp_status=0");
                  xhttp.send(); 
      ...
      Swal.fire({
          position: 'center',
          icon: warning,
          title: 'Please select atleast one task',
